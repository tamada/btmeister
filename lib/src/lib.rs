...
impl MeisterError {
    fn error_or_item<T, F: FnOnce() -> T>(item: F, errs: Vec<MeisterError>) -> Result<T> {
        if errs.is_empty() {
            Ok(item())
        } else if errs.len() == 1 {
...
impl Meister {
    ...
    /// If `its` was the empty, the default value ([IgnoreType::Default]) will be used.
    /// The `excludes` is a list of patterns for excluding paths in traversing. 
    ...
    fn find_directory(&self, base: PathBuf) -> Result<BuildTools> {
        ...
        MeisterError::error_or_item(|| BuildTools { base, tools: result }, errs)
    }
}
...
fn build_matcher(def: BuildToolDef) -> Result<MultipleMatcher> {
    ...
    MeisterError::error_or_item(|| MultipleMatcher { matchers }, errs)
}
...
fn build_matchers(defs: Vec<BuildToolDef>) -> Result<Vec<MultipleMatcher>> {
    ...
    MeisterError::error_or_item(|| result, errs)
}
